...
export function ChatMessage({ message, onConfirmAction, onRejectAction }: ChatMessageProps) {
  ...
  // Parse tool calls for display info
  const toolCallsData = hasToolCalls ? JSON.parse(message.toolCalls!) : [];
  ...
  return (
    ...
    >
      ...
      {/* Action cards */}
      {actions.map((action) => {
        const toolCall = toolCallsData.find((tc: { toolName?: string; entityType?: string }) =>
          tc.toolName?.toLowerCase().includes(action.entityType) ||
          tc.entityType === action.entityType
        );
        ...
        return (
          ...
          >
            <p className="text-sm font-medium mb-2">
              {action.status === 'confirmed' && <Check className="h-4 w-4 inline mr-1 text-green-600" />}
              {action.status === 'rejected' && <X className="h-4 w-4 inline mr-1 text-red-600" />}
            ...
            {action.status === 'rejected' && (
              <p className="text-xs text-red-600">Action cancelled</p>
            )}
          </Card>
        );
      })}
